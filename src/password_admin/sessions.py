...
class SessionStore:
    ...
    def create_session(self, credentials: LoginCredentials) -> str | None:
        ...
        while session_id in self.__store:
            session_id = self.__create_random_session_id()
        self.__store[session_id] = database_connection
    ...
    def destroy_session(self, session_id: str) -> None:
        ...
        try:
            database_connection = self.__store.pop(session_id)
        except KeyError:
            return
        else:
    ...
    def get_database_connection(self, session_id: Annotated[str | None, Cookie()] = None) -> DatabaseInterface:
...
session_store = SessionStore(DatabaseFactory(settings.database_type))

ActiveDatabaseConnection = Annotated[DatabaseInterface, Depends(session_store.get_database_connection)]
