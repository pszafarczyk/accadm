...
class DatabaseConnectionPostgres:
    ...
    Provides methods to configure, connect, query, and modify data in a PostgreSQL
    database using the psycopg2 library. Supports user authentication, retrieval of
    user identifiers, and password updates. Logs operations and errors using the
    Python logging module.

    Attributes:
        connection (psycopg2.connection): The active connection to the PostgreSQL
            database.
    ...
    def __init__(self, config: PostgresConfig):
        ...
        Sets up the initial state with no connection or cursor established. The
        `config` method is called with the provided configuration to prepare the
        database connection. Initializes the logger.

        Args:
            config (PostgresConfig): Configuration object containing database
                connection details (e.g., DSN).
        """
    ...
    def config(self, config: PostgresConfig) -> None:
        ...
        Stores the provided configuration for use in establishing a database
        connection.
        ...
        Raises:
            password_admin.exceptions.DbConnectionError: If the configuration fails
        ...
        try:
            ...
            self.logger.exception('Configuration error: Invalid configuration')
            raise password_admin.exceptions.DbConnectionError(detail=f'Configuration error: Invalid configuration: {e!s}') from None

    def login(self, credentials: LoginCredentials) -> None:
        """Establishes a connection to the PostgreSQL database.

        Connects to the database using the provided credentials and configuration.
        Creates a cursor for executing queries. The configuration must be set using
        `config` before calling this method.

        Args:
            credentials (LoginCredentials): Object containing the username and
                password for authentication.

        Raises:
            password_admin.exceptions.DbLoginError: If the connection fails (e.g.,
                invalid credentials or database unreachable).
        """
        try:
    ...
    def get_users(self) -> list[str]:
        ...
        Executes the configured query to fetch user identifiers from the specified
        table. Assumes the query returns a single column of user identifiers.
        ...
        Raises:
            password_admin.exceptions.DbConnectionError: If no connection or cursor is
                established (i.e., `login` was not called).
            password_admin.exceptions.DbQueryError: If the query execution fails
                (e.g., invalid query or database issues).
        """
        try:
            ...
            self.logger.info('Retrieved %s users from PostgreSQL database', len(users))
            return users
            ...
            self.logger.exception('Error retrieving users')
            raise password_admin.exceptions.DbConnectionError(detail=f'Error retrieving users: {e!s}') from None
    ...
    def set_password(self, credentials: NewCredentials) -> None:
        ...
        Returns:
            bool: True if the password update is successful, False if no rows are
                affected or an error occurs.

        Raises:
            password_admin.exceptions.DbConnectionError: If no connection or cursor
                is established (i.e., `login` was not called).
            password_admin.exceptions.DbQueryError: If the query execution fails
                (e.g., invalid query or database issues).
            password_admin.exceptions.SessionNotFoundError: If no user is found with
        ...
        try:
            ...
            if success:
                self.logger.info('Successfully updated password for user: %s', credentials.username)
            else:
                self.logger.warning('No user found with identifier: %s', credentials.username)
            ...
            raise password_admin.exceptions.DbQueryError(detail=f'Error updating password: {e!s}') from None
        except (password_admin.exceptions.DbConnectionError, password_admin.exceptions.SessionNotFoundError):
            raise
        except ValueError as e:
            self.logger.exception('Error updating password')
            raise password_admin.exceptions.DbConnectionError(detail=f'Error updating password: {e!s}') from None
    ...
    def logout(self) -> None:
        ...
        Raises:
            password_admin.exceptions.DbConnectionError: If closing the connection or
                cursor fails (e.g., database issues).
        """
        try:
            if self.cursor:
                self.cursor.close()
